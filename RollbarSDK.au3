...
If StringRight(@ScriptName, 4) = ".au3" Then Global $__G_Rollbar_bDebug = True ; Comment line out to disable debugging when ran as .au3 file. (click on line, then press numpad minus)
; ===============================================================================================================================
...
Func _Rollbar_Init($__Rollbar_sToken)
	...
	Select
		Case $__G_Rollbar_sToken = ""
			Return False
		Case Else
			If $__G_Rollbar_bDebug = True Then ConsoleWrite("[" & $__G_Rollbar_Strings_Console_sTitle & "] " & $__G_Rollbar_Strings_Console_sInit & @CRLF)
			Return True
	EndSelect
...
Func _Rollbar_Init_Ask($__Rollbar_sToken)
	...
	Select
		...
		Case IniRead(@ScriptDir & "\" & StringTrimRight(@ScriptName, 4) & "_" & $__G_Rollbar_Strings_IniFile_sDebugging & ".ini", $__G_Rollbar_Strings_Ini_sSection, $__G_Rollbar_Strings_Ini_sKey, "") = "False"
			$__G_Rollbar_sToken = ""
			If $__G_Rollbar_bDebug = True Then ConsoleWrite("[" & $__G_Rollbar_Strings_Console_sTitle & "] " & $__G_Rollbar_Strings_Console_sInitOptOut & @CRLF)
			Return False
...
Func _Rollbar_SendItem($__Rollbar_sJSONData)
	...
	If $__G_Rollbar_bDebug = True Then ConsoleWrite($__G_Rollbar_Strings_Console_sPostRequest & " " & $__Rollbar_sJSONData & @CRLF)
	...
	If $__G_Rollbar_bDebug = True Then ConsoleWrite($__Rollbar_sReceived & @CRLF)

	; Process Response Code
	Switch $__Rollbar_iReceived
		Case 200
			; Good job!
			If $__G_Rollbar_bDebug = True Then ConsoleWrite("[" & $__G_Rollbar_Strings_Console_sTitle & "] " & $__G_Rollbar_Strings_Console_sHTTP200 & @CRLF)
			Return SetError(False, 200, True)

		Case 400
			; Bug in SDK
			If $__G_Rollbar_bDebug = True Then ConsoleWrite("[" & $__G_Rollbar_Strings_Console_sTitle & "] " & $__G_Rollbar_Strings_Console_sHTTP400 & @CRLF)
			Return SetError(True, 400, False)

		Case 403
			; Bug in Program
			; $__G_Rollbar_sToken is either not set or invalid. See Function _Rollbar_Init()
			If $__G_Rollbar_bDebug = True Then ConsoleWrite("[" & $__G_Rollbar_Strings_Console_sTitle & "] " & $__G_Rollbar_Strings_Console_sHTTP403 & @CRLF)
			Return SetError(True, 403, False)

		Case 404
			; Bug in SDK
			If $__G_Rollbar_bDebug = True Then ConsoleWrite("[" & $__G_Rollbar_Strings_Console_sTitle & "] " & $__G_Rollbar_Strings_Console_sHTTP404 & @CRLF)
			Return SetError(True, 404, False)

		Case 413 ; Bug in Program.
			If $__G_Rollbar_bDebug = True Then ConsoleWrite("[" & $__G_Rollbar_Strings_Console_sTitle & "] " & $__G_Rollbar_Strings_Console_sHTTP413 & @CRLF)
			Return SetError(True, 413, False)

		Case 422
			; Bug in SDK
			If $__G_Rollbar_bDebug = True Then ConsoleWrite("[" & $__G_Rollbar_Strings_Console_sTitle & "] " & $__G_Rollbar_Strings_Console_sHTTP422 & @CRLF)
		...
		Case 429 ;
			...
			If $__G_Rollbar_bDebug = True Then ConsoleWrite("[" & $__G_Rollbar_Strings_Console_sTitle & "] " & $__G_Rollbar_Strings_Console_sHTTP429 & @CRLF)
			Return SetError(True, 0, False)

		Case Else
			; Bug in SDK.
			If $__G_Rollbar_bDebug = True Then ConsoleWrite("[" & $__G_Rollbar_Strings_Console_sTitle & "] " & $__G_Rollbar_Strings_Console_sHTTPResponse & " " & $__Rollbar_iReceived & ". " & $__G_Rollbar_Strings_Console_sHTTPUndocumented & @CRLF)
